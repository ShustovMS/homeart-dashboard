...
def build_sheet_methodology(wb):
    ...
    # --- Блок: ключевые слова для определения категории ---
    r5 = r4 + len(cat_rows) + 4
    ws.cell(row=r5, column=1,
            value="Ключевые слова для определения категории (по колонке «Вид товара»)"
            ).font = SUBTITLE_FONT
    ws.merge_cells(f"A{r5}:D{r5}")

    r5 += 1
    ws.cell(row=r5, column=1,
            value="Каждое значение из поля «Вид товара» разбивается по запятой на части. "
                  "Каждая часть проверяется на вхождение ключевых слов (регистр не важен). "
                  "«Технический свет» проверяется раньше «Свет», чтобы не попасть в «Свет декоративный»."
            ).font = Font(name="Arial", italic=True, size=9, color="4472C4")
    ws.merge_cells(f"A{r5}:D{r5}")

    r5 += 2
    kw_headers = ["Категория", "Ключевые слова (если слово входит в часть — относим к этой категории)", "Примеры из CRM", ""]
    for c, h in enumerate(kw_headers[:3], 1):
        ws.cell(row=r5, column=c, value=h)
    apply_header_style(ws, r5, 3)

    kw_rows = [
        ("Мебель",
         "мебель, гардероб, уличная мебель, матрас, ковры",
         "Мебель / Гардероб / Уличная мебель / Мебель, Ковры / Мебель, Гардероб"),
        ("Кухни",
         "кухня, кухн",
         "Кухня"),
        ("Сантехника",
        ...
        ("Двери",
         ...
         "Двери"),
        ("Свет декоративный",
         "свет, декоратив  (но только если НЕТ слова «технический»)",
         "Свет / Свет, Технический свет → часть «Свет» → Свет декоративный"),
        ("Свет технический",
         "технический свет, техсвет  (проверяется ПЕРВЫМ, до «свет»)",
         "Технический свет / Свет, Технический свет → часть «Технический свет» → Свет технический"),
        ("Столярка",
         "столярка, столяр, индивидуальное производство",
         "(в текущей выгрузке не встречается)"),
        ("Другие",
         "всё, что не попало ни в одну категорию выше",
         "Техника / Розетки / Отделочные материалы / Другое / Аксессуары"),
    ]

    for i, (cat, keywords, examples) in enumerate(kw_rows):
        row = r5 + 1 + i
        ws.cell(row=row, column=1, value=cat)
        ws.cell(row=row, column=2, value=keywords)
        ws.cell(row=row, column=3, value=examples)
        apply_data_style(ws, row, 3, is_alt=(i % 2 == 1))

    # --- Блок: маппинг реальных значений из выгрузки ---
    r6 = r5 + len(kw_rows) + 4
    ws.cell(row=r6, column=1,
            value="Маппинг реальных значений «Вид товара» → категория(и)"
            ).font = SUBTITLE_FONT
    ws.merge_cells(f"A{r6}:D{r6}")

    r6 += 1
    ws.cell(row=r6, column=1,
            value="Таблица показывает, в какие категории попадает каждое конкретное значение из CRM. "
                  "Если значение содержит несколько позиций через запятую — сделка считается в каждой категории."
            ).font = Font(name="Arial", italic=True, size=9, color="4472C4")
    ws.merge_cells(f"A{r6}:D{r6}")

    r6 += 2
    map_headers = ["Значение в CRM («Вид товара»)", "Попадает в категории", "Кол-во сделок в выгрузке", "Примечание"]
    for c, h in enumerate(map_headers, 1):
        ws.cell(row=r6, column=c, value=h)
    apply_header_style(ws, r6, len(map_headers))

    mapping_rows = [
        ("Мебель",               "Мебель",                                    47, ""),
        ("Свет",                 "Свет декоративный",                          20, ""),
        ("Кухня",                "Кухни",                                      13, ""),
        ("Сантехника",           "Сантехника",                                 11, ""),
        ("Мебель, Свет",         "Мебель + Свет декоративный",                 10, "Сделка считается в обеих категориях"),
        ("Двери",                "Двери",                                       5, ""),
        ("Гардероб",             "Мебель",                                      4, "«Гардероб» входит в Мебель"),
        ("Отделочные материалы", "Другие",                                      4, "Нет совпадений с ключевыми словами"),
        ("Техника",              "Другие",                                      3, ""),
        ("Розетки",              "Другие",                                      2, ""),
        ("Мебель, Ковры",        "Мебель",                                      1, "Оба слова → Мебель, дублей нет"),
        ("Технический свет",     "Свет технический",                            1, "Проверяется до «Свет декоративный»"),
        ("Свет, Технический свет","Свет декоративный + Свет технический",       1, "Каждая часть — отдельная категория"),
        ("Мебель, Гардероб",     "Мебель",                                      1, "Оба слова → Мебель"),
        ("Мебель, Свет, Сантехника","Мебель + Свет декоративный + Сантехника",  1, ""),
        ("Мебель, Свет, Сантехника, Гардероб","Мебель + Свет декоративный + Сантехника", 1, "Гардероб → Мебель (уже есть)"),
        ("Мебель, Другое",       "Мебель + Другие",                             1, "«Другое» не попало в категории"),
        ("Уличная мебель",       "Мебель",                                      1, "Ключевое слово «уличная мебель»"),
        ("Аксессуары",           "Другие",                                      1, "Нет совпадений → Другие"),
        ("Другое",               "Другие",                                      1, ""),
        ("Мебель, Свет, Сантехника, Кухня, Гардероб,\nДвери, Уличная мебель, Ковры, Технический свет",
         "Мебель + Свет декоративный + Сантехника +\nКухни + Двери + Свет технический",
         1, "Все 6 категорий из одной сделки"),
        ("(не заполнено)",       "Другие",                                      "—", "Пустое поле → Другие"),
    ]

    for i, (val, cats, cnt, note) in enumerate(mapping_rows):
        row = r6 + 1 + i
        ws.cell(row=row, column=1, value=val)
        ws.cell(row=row, column=2, value=cats)
        ws.cell(row=row, column=3, value=cnt)
        ws.cell(row=row, column=4, value=note)
        apply_data_style(ws, row, len(map_headers), is_alt=(i % 2 == 1))
...
CATEGORY_MAP = [
    ("Мебель",              ["мебель", "гардероб", "уличная мебель", "матрас", "ковры"]),
    ("Кухни",               ["кухня", "кухн"]),
