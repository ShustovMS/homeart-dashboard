...
def build_sheet_methodology(wb):
    ...
    rows_data = [
        ...
        (
            "Дошло до предоплаты",
            "Кол-во сделок, текущий этап которых >= «Получение предоплаты» (ранг 8+)",
            "Колонка [6] Этап сделки",
            "Логика: если сделка сейчас на этапе после предоплаты (напр. оплата проформы, "
            "заказ у фабрики, успешно реализовано), значит она прошла предоплату. "
            "Сделки «на стопе» (ранг 0) и «не реализованные» (ранг -10) НЕ считаются дошедшими"
        ),
        (
